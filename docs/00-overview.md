...
KJS 引擎做的事情，说到底就是一件事：把你写下的 JavaScript 代码，变成屏幕上的运行结果。中间要过几道工序——先把源码"切词"（词法分析），再整理成"结构树"（语法分析），然后翻译成一份给虚拟机照着执行的"字节码清单"（编译），最后由虚拟机一条条算出来（执行）。本章先用 `1+2*3` 这个最小例子，把整条管线走一遍（见 §3），让你先有整体画面，再往后各章逐个拆开讲细。
