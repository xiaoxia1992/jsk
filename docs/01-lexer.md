...
> **写给小白（本章导读）**：这一章讲"词法分析器（Lexer）"——引擎流水线上的第一道工序，把你写的一长串字符（例如 `var x = 1 + 2;`）切成一串"带类型的小块"，术语叫 **Token（词）**。
> - **基础信息**：把 Lexer 想成"给代码分词的分词器"，类似把"我爱编程"分成"我/爱/编程"，但它还会给每个词贴"类型标签"（数字？名字？加号？）并记住它在第几行第几列，方便报错定位。
> - **别的方案对比**：有的语言用"正则表达式一把梭"切词（短但难处理 `/` 这种歧义）；有的用"词法生成器"（如 flex）自动生成切词代码。KJS 选"手写带光标的扫描器"，逻辑全在眼前、好调试，代价是代码长一点。
> - **进阶**：§7 的 `/` 歧义是 JS 最经典的坑，§12 设计取舍对比了和别的引擎的做法。
...
> **小白讲解**：**Token** 就是"切出来的最小有意义词块"，并贴了张类型标签。比如 `123` 是数字词、`x` 是标识符词、`+` 是运算符词。下面这张清单把所有词的类型列全了，不必死记，当字典用即可。
...
> **小白讲解**：JS 里 `/` 既可能是除法 `a/b` 也可能是正则 `/foo/g`，光看字符分不出来，必须看它前面是什么。KJS 用极轻量的办法：记住"上一个词的类型"来判断。这是 JS 词法最经典、最易写错的坑，下面用判定表讲清每类词后 `/` 该当除号还是正则。 **别的方案**：有的引擎让语法分析器也参与切词（词法语法不分家），但边界就糊了；KJS 让 Lexer 自己用一个状态搞定，职责更清晰。
...
> **小白讲解**：这一节带你"单步跟踪"真实输入 `a / b; /re/g.test(x)`，看同一个 `/` 在第 3 步被切成除号、第 8 步被切成正则，只因前面的词不同。跟着表格走一遍，§7 的判定规则就彻底活了。
...
> **小白讲解**：这节总结"作者为什么这么设计"并和别的做法对比。重点：① 词法阶段就把数字直接算成值（避免后面重复算）；② 模板串里 `${...}` 这种复杂东西丢给后面的语法分析器（各司其职）。 **进阶**：想看真实报错怎么定位，跳 §10 `LexError`。
