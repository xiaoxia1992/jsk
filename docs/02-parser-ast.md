...
语法分析器（Parser）是流水线的第二道工序。Lexer 把字符切成词（Token）之后，Parser 再按 JS 的语法规则，把这些词拼成一个"结构树"，也就是 AST（抽象语法树）——就像把"主谓宾"组织成句子结构。所谓 AST，就是代码的树形结构表示：`1 + 2 * 3` 在 AST 里是一棵"加法节点，左孩子是 1、右孩子是乘法节点（2 和 3）"的树，运算顺序就藏在树的结构里（先乘后加）。解析语法有两大流派——"递归下降"和"自动生成"（用 ANTLR / Bison 之类工具从文法自动生成解析器）。KJS 选了递归下降：手写、直观、好调试，更重要的是它能边解析边"解语法糖"（见 §4）。全篇最精妙的是 §3 的"优先级爬升"——它决定了 `1+2*3` 为什么先算乘法；§4 则讲箭头函数、`for-of`、模板这些"糖"是怎么在解析阶段就被展开的。
...
AST 节点就是这棵树上的"零件"。`sealed class` 是 Kotlin 的一种写法，作用是把节点的所有可能种类都列在明处，编译器就能帮你检查有没有漏掉某种节点的处理。下面把语句、表达式的各种节点一一列出，可以当成目录随时翻看。
...
这一节是全篇最硬核也最精彩的地方。所谓"优先级爬升"，是一种不用写一大堆 if 嵌套、就能正确处理运算符先后的算法：每一层函数只管比自己低一级的运算符，遇到同级就一直往左收。以 `1+2*3` 为例，`+` 那一层发现右边是 `*`（优先级更高），便先让 `*` 那一层去算 `2*3`，再把结果拿回来相加。文末的 mermaid 图把层级从低到高画了出来。值得一提的是：`**` 是右结合（`2**3**2` 等于 `2**(3**2)`），而 `*` 是左结合——这个细节算法里是专门处理的。
...
所谓"语法糖"，就是看着甜、写起来省事，但本质上能被翻译成更基础写法的语法。比如箭头函数 `(x)=>x*2` 本质就是个普通函数；`for (x of arr)` 本质是反复调用"取下一个元素"的迭代器。当 `for-of` 碰到解构（`for ({a} of arr)`）时，Parser 会偷偷塞一个临时变量把解构展开，后面的编译器就不必再操心这件事。
...
这一节总结设计取舍。KJS 选择"递归下降 + 在解析阶段就把语法糖展开"，好处是后面的编译器那道工序更轻松，代价是 Parser 本身稍复杂一些。顺带说一句：也有引擎把"解糖"放到编译器、甚至专门的"解糖器"里做——没有绝对的对错，只是职责划分不同。
