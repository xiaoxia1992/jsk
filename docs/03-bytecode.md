...
字节码是虚拟机照着一步步执行的"简单指令清单"，可以把它想成一份半成品菜谱。在 KJS 里，这份清单存放在电脑中，形式上是三条并排的数字列（就像 Excel 的三列：第一列记"动作"、第二三列记"参数"），另外还有几个"池子"用来存放可复用的常量、字符串和子函数。全篇最该收藏的是后面的"逐指令参考表"——它把每条指令对"操作数栈（盘子）"和"局部变量（格子）"的精确操作列得清清楚楚。两个最可能卡住的术语先提一句：**emit** 指"往清单末尾加一行指令"，**跳转修补** 指"跳转目标还不知道时先占个坑、等知道了再回填"，第 3、5 节会细讲。
...
字节码本质上就是"给虚拟机看的机器指令清单"。JS 源码被编译之后，会变成一串很简单的指令，比如"把数字 3 压进栈""把两个数相加"。KJS 把这些指令存成三个并排的数组：第一张只记"每个步骤干啥"（操作类型），第二、三张记每个步骤需要的"参数"。三张数组用同一个行号对齐——同一个行号，对应同一条指令——这正是"三条并行"这个名字的由来。
>
...
`emit` 就是"往清单末尾加一行指令"。麻烦出在 `if`/`for` 这类带跳转的指令上：它要跳到的"目标行号"，在写下这条指令时还根本不存在（目标在更后面才会生成）。于是编译器先填一个占位数字（比如 0），等后面知道了真正的行号，再用 `patchA(坑的行号, 真正目标)` 把占位改对。这就像写菜谱时先写"转到第 ___ 步"，等后面的步骤写好了，再回头把空白处填上。`patchB` 同理，只是改第二列的参数——比如 `try/catch` 要同时记下 catch 和 finally 两个位置。
...
这是全篇最该收藏的一张表。每条指令占一行，列了四件事：这条指令干啥、它的 A/B 操作数是什么、对"操作数栈（盘子）"做了什么（记法 `[…, x, y] → […, z]`，`…` 是栈底、右边是栈顶）、对局部变量格子 / 常量池 / 环境有没有副作用。读到某条指令拿不准时，先来这里查。还有一个容易忽略的点：KJS 里的"赋值"是一个表达式——`STORE_*` 写完值之后会把值留在栈顶，所以独立的赋值语句通常要再跟一条 `POP` 把残留清掉。
...
这一节讨论"为什么用三条并行数组，而不是别的存法"。两种常见替代方案是：把每条指令做成一个对象（代码好看，但慢、费内存），或者用一条连续的字节流、让参数紧跟在操作码后面（V8 的 Ignition、QuickJS 都这么做，紧凑，但运行时得先解析"这条指令占几个字节"）。KJS 选三条等长的数组，是为了让 `when(op)` 的分发整齐统一，代价是内存比单字节流略胖一些。
