...
> **白话导读（给第一次读的人）**：字节码就是 VM 照着一步步执行的"简单指令清单"，像一份中间菜谱。它在电脑里存成**三条并排的数字列**（就像 Excel 的三列：第一列记"动作"、第二三列记"参数"），另外还有几个"池子"存复用的常量/字符串/子函数。整篇最该收藏的是后面的"逐指令参考表"——它把每条指令对"盘子（操作数栈）"和"格子（局部变量）"的精确操作列得清清楚楚。最难懂的两个词是 **emit**（往清单末尾加一行）和**跳转修补**（跳转目标还不知道时先占个坑、等知道了再填），第 3、5 节会细讲。
...
> **小白讲解**：`emit` = "往清单末尾加一行指令"。麻烦出在 `if`/`for` 这种带跳转的指令——它要跳到的"目标行号"在写它时还**不存在**（目标在后面才生成）。于是先填一个**占位**数字（比如 0），等后面知道了真正行号，再用 `patchA(坑的行号, 真正目标)` 改对。就像写菜谱先写"转到第 ___ 步"，等后面步骤写好再回去填空。`patchB` 同理，改第二列参数（如 `try/catch` 同时记 catch、finally 两个位置）。
...
> **小白讲解**：这是全篇最该收藏的一张表。每条指令占一行，列了四件事：**这条指令干啥**、**它的 A/B 操作数是啥**、**对"盘子（操作数栈）"做了什么**（记法 `[…, x, y] → […, z]`，`…` 是栈底、右边是栈顶）、**对局部变量格子/池子/环境有没有副作用**。读不懂某条指令时，先来这查。注意 KJS 的"赋值"是表达式——`STORE_*` 写完值后**把值留在栈顶**，所以独立语句通常后面跟一条 `POP` 清掉残留。
...
> **小白讲解**：这节讲"为什么用三条并行数组，而不是别的存法"。 **别的方案对比**：① 每条指令做成对象（代码好看但慢、费内存）；② 单条字节流、参数紧跟操作码（V8 Ignition、QuickJS 这么干，紧凑但要解析"这条占几个字节"）。KJS 选三条等长数组，是为让 `when(op)` 分发整齐统一，代价是内存比单流略胖。
