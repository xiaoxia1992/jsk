...
编译器只干一件事：把语法树翻译成字节码清单。有四个核心概念最好先建立直觉，后面各节都会带着源码展开：一是"槽"——变量名只是给人看的，机器只认编号，编译器把每个局部变量分配到"第几号格子"；二是"闭包"——内层函数记住外层变量，在 KJS 里实现成一个共享盒子；三是"提升"——进入函数时先把所有 `var` / 函数声明登记好，所以能"先调用后声明"；四是"跳转修补"——`if`/`for` 的跳转目标还不知道时先占坑、知道了再填。
...
"槽位（slot）"就是给局部变量编的"第几号格子"。在 JS 里，变量名是写给人看的，机器只认编号。编译器扫描一遍函数体，把 `var` / `let` / 参数依次分配到 `locals[0]`、`locals[1]`……，之后生成的字节码里就只写"读第 0 号格子、写第 1 号格子"。这样一来，栈式虚拟机不必记名字，直接按编号存取，又快又省。
...
这是编译器四个核心原理里最容易绕晕的一个，但本质其实简单：写 `if`/`for` 的跳转指令时，目标行号还不知道，于是先 `emit` 一个占位跳转（目标先填 0）；等"then 分支 / 循环体"编译完、知道结尾在第几行之后，再用 `patchA` 把占位回填成真实行号。比喻见 D3 §3——就像写菜谱时先写"转到第 ___ 步"，后面补上。
