...
>
> 本篇的目标：**让你读完能在脑中单步执行任意一段字节码**。所以我们先不讲指令，
> 而是先建立 VM 的心智模型——**Frame 是什么、一次调用发生了什么、参数和返回值怎么流动、
> 局部变量表长什么样**——再用一个具体例子把整条链路 trace 一遍，最后才逐主题拆解 opcode。

---

## 0. 心智模型：VM 是怎么"跑"起来的

KJS 的 VM 是一个**栈式解释器**：源码 →（编译器 D4）→ 字节码 `Bytecode` → VM 逐条执行。

它最关键、也最常被误解的一点是：

> **VM 不是"一个循环跑完所有代码"，而是"每调用一次函数，就新建一个 `Frame`，
> 然后在该 `Frame` 上跑分发循环；函数返回时退出该 `Frame`。"调用"= Kotlin 的一次递归。"**

也就是说，JS 的调用栈 = Kotlin 的调用栈。如果你在 `add(3,4)` 里又调 `mul()`，那么调用栈是：
...
所以回答你的第一个问题：**是的，执行过程中 Frame 不断地被创建和退出**。每进入一次函数调用
就 `new` 一个 `Frame`（内部 `Array` 复用，见 §2），函数 `RET` 时就退出、把返回值交还上层 `Frame`。

整个 VM 只有一类循环（`runLoop` 里的 `while(true)`），所有 opcode 都在这里面分发执行；
而"函数调用"只是从循环内部再递归进一个新的 `runLoop` + 新 `Frame`。
...
`Frame`（`Vm.kt:50`）是**一次函数调用（一次函数执行）的全部运行时状态**。它把所有"要让这个函数
跑起来且中途不串味"的东西打包在一起：
...
private class Frame(
    ...
    // 异常处理器栈: 每项为 [catchPc, finallyPc, enterSp] 三元组（见 §10）
...
| Frame 字段 | 对应 JS 概念的什么 |
|---|---|
| `bc` | 函数体编译后的字节码 |
| `stack` | 求值用的操作数栈（如算 `a+b` 时先压 `a`、`b`，再加法弹两个压结果） |
| `locals` | 局部变量表（参数在最前面，见 §3 / §4） |
| `thisVal` | `this` |
| `args` | `arguments` 的底层数组 |
| `upvalues` | 闭包捕获的外部环境变量（见 §5） |
| `pc` | "现在执行到第几条指令" |
| `sp` | "操作数栈用了多高" |
| `handlers` / `pendingThrow` | `try/catch/finally` 所需的控制流状态 |

一句话：**一个 `Frame` = 一个正在执行的函数的"全部家当"。** VM 不靠全局变量保存执行状态，
而是把状态封进 `Frame`，这样多函数嵌套调用时各跑各的、互不干扰。

> 补充：`Frame` 是 VM 的**私有内部类**，外部拿不到。JS 层看到的"函数对象"是 `JsFunction`
> （D6），它里面挂着一个 `VmClosure`（`Vm.kt:14`）——`VmClosure` 才是"可被执行的函数载体"，
> 携带 `Bytecode`、`closureEnv`、`Upvalue[]` 以及 JIT 状态（`hotness`/`compiled` 等）。
> 每次调用 `VmClosure` 时，再由它造出一个 `Frame` 去真正执行。

---

## 2. 调用即"建帧"：Frame 如何被创建与退出（兼答"是不是不断生成退出"）

每次调用都经过 `execClosureArr`（`Vm.kt:132`），它干三件事：**建 Frame → 进循环执行 → 退 Frame 还池**。
...
- **确实"每次调用都新建 Frame"**：`new Frame(...)` 每次调用都发生，`Frame` 对象本身不池化
  （它很轻，只是个引用容器）；但 `stack`/`locals` 两个大数组从 `stackPool`/`localsPool`
  （`Vm.kt:107`）借出、用完归还，**避免每次调用都 `new` 大数组**。
- **`pop`/`release` 会置 null**（`Vm.kt:117`）：归还前清空数组元素，防止旧引用残留在池化内存里
  造成"隐式内存泄漏"。
- **`finally` 保证退帧**：无论函数是正常 `RET` 还是抛异常，都会走 `releaseStack/releaseLocals`
  把数组还池。所以热递归（如 `fib(30)`）下，调用的"昂贵部分"（分配数组）被池化抹平，GC 压力骤降——
  这是 VM 比"递归解释 AST"快很多的主因之一。池上限各 64 个，超出则直接丢弃让 GC 收。
...
sequenceDiagram
    ...
    participant Loop as runLoop(被调)
    Caller->>EX: 传 thisVal + argsArr
    EX->>Pool: borrowStack / borrowLocals
    EX->>Loop: new Frame + 绑定参数 → runFrame
    Loop->>Loop: while(true) 执行指令…
    Loop-->>EX: RET 返回值
...
## 3. 参数怎么传进来：调用约定（兼答"参数传递"）

参数传递分两端：**调用端**把参数从操作数栈收进 `argsArr`；**被调端**把 `argsArr` 绑进局部变量表。

### 3.1 调用端：CALL 把栈上的参数收成数组

调用约定约定：调用前，实参按 `arg0, arg1, … argN-1` 的顺序压在操作数栈上，紧接着压被调函数。
...
为什么**逆序弹出**？因为栈是 LIFO：最后压进去的是 `argN-1`（在栈顶），先弹出来正好放进
`argsArr[N-1]`，于是 `argsArr[0]` 始终是 `arg0`。弹出顺序和数组下标"对上了"。

### 3.2 被调端：实参写入局部变量表

回到 §2 的建帧代码：`frame.locals[i] = if (i < argsArr.size) argsArr[i] else UNDEFINED`。
即 `argsArr[0..paramCount-1]` 被依次复制到 `locals[0..paramCount-1]`。**少传的参数自动是 `undefined`**
（标准 JS 语义，如声明 3 个参数只传 2 个，第 3 个 `locals[2] = undefined`）。

### 3.3 this 怎么绑

- `CALL`：非方法调用，`this = realm.globalObject`（非严格默认全局对象）。
- `CALL_METHOD`（`Vm.kt:455`）：从栈上再弹一个"方法所属对象"作为 `this`；
  **但若函数是箭头函数**（`fn.getOwn("__arrow__")==true`），则沿用外层 `f.thisVal`——
  因为箭头函数不重新绑定 `this`（JS 语义）。
- `NEW_OP`（`Vm.kt:464`）：先按 `ctor.prototype` 造一个新对象作 `this`，构造函数返回对象则用返回值覆盖。

---

## 4. 函数的局部变量表是什么（兼答"局部变量表"）

`locals: Array<Any?>` 就是**局部变量表**，一个按"槽号（slot）"索引的扁平数组。
...
- **槽 0..paramCount-1 = 参数**：建帧时直接绑定（§3.2）。
- **槽 paramCount..localCount-1 = 局部变量和编译器临时变量**：由编译器在 D4 里统一编号分配。
  比如函数里声明了 `let x`，编译器给它分配一个空闲槽；表达式求值的中间结果也可能占用临时槽。
- **总大小** `localsSize = maxOf(bc.localCount, bc.paramCount) + 4`（`Vm.kt:175`），`+4` 是安全余量。

### 4.2 LOAD_LOCAL / STORE_LOCAL
...
> **关键细节**：`locals[a]` 里存的**不一定是值，可能是 `Upvalue` 盒子**（当这个局部被内层闭包捕获时）。
> 若是盒子，读写都走 `box.value`——于是父帧和所有捕获它的闭包**共享同一份内存**（见 §5）。

### 4.3 参数 vs arguments：args 与 locals 的区别

`Frame` 同时有 `args`（原始实参数组）和 `locals[0..paramCount-1]`（参数的副本绑定）。两者用途不同：

- `locals` 里的是"形参变量"，`STORE_LOCAL` 改它不会改 `args`。
- `args` 是 `arguments` 的底层数据。`LOAD_ARGUMENTS`（`Vm.kt:478`）把 `f.args` 整包成数组返回；
  `LOAD_ARG a` / `STORE_ARG a`（`Vm.kt:254`）直接按下标读/写 `f.args`——这正是 `arguments[i]`
  的实现。例：`function f(a){ arguments[0] = 9; return a }`，调 `f(1)` 返回 `9`，因为 `arguments`
  和形参 `a` 在早期 JS 语义里共享存储（本 VM 通过 `args` 数组实现这一可见行为）。

---

## 5. 返回值怎么传出去（兼答"返回值传递"）

返回值只有一条规则：**被调函数从自己的操作数栈弹出栈顶，直接 `return` 出 `runLoop`；
该返回值沿调用链回到 `CALL` 指令，被压入调用者的操作数栈。**
...
串起来看（对照 §2 的 `finally`）：`RET → runLoop` 返回 → `runFrame` 的 `try/finally` 释放数组 →
`execClosureArr` 把返回值返回 → 回到调用者的 `CALL` 指令 `f.push(invokeFast(...))`——**返回值压进
调用者栈，调用者继续往下执行**。顶层程序则用 `STASH_RESULT`/`HALT`（`Vm.kt:519`）把最后结果存到
`frame.lastResult`，`run` 拿到它作为 `eval` 的返回值。

---

## 6. 一个完整 trace：`add(3, 4)`

把上面 §2~§5 串起来，看这段代码的字节码是怎么跑的：
...
**阶段 A — 顶层帧执行到 `add(3,4)` 调用前**：操作数栈上已依次压入 `add` 函数对象、`3`、`4`。
...
4. 建帧时绑定参数：`addFrame.locals[0]=3`，`addFrame.locals[1]=4`，`thisVal = globalObject`。
...
**阶段 D — 返回**：`7` 从 `runLoop(add)` 一路回到顶层 `CALL`，`f.push(7)`。顶层栈变 `[7]`。
随后 `HALT` 把 `7` 作为顶层结果返回给 `run`。
...
**一句话总结链路**：调用者把参数压栈 → `CALL` 逆序收成 `argsArr` → 建被调 Frame 并把 `argsArr`
绑进 `locals[0..n]` → 被调执行 → `RET` 弹栈顶 → 返回值压回调用者栈。

---

## 7. 主分发循环：runFrame / runLoop

现在进入"单函数内部是怎么逐条执行的"。所有执行都收敛到 `runFrame`（`Vm.kt:207`）→ `runLoop`
（`Vm.kt:220`）。`runLoop` 是一个**永真 `while` + `when(op)`**：
...
- **一个 Frame 对应一个 `runLoop`**：函数体内的所有指令都在同一个 `while` 循环里顺序/跳转执行，
  直到遇到 `RET`（返回）或 `HALT`（顶层结束）。

---

## 8. 闭包与 Upvalue（真正的"变量捕获"机制）

### 8.1 MAKE_CLOSURE：open upvalue 共享盒子
...
两种 upvalue 来源：
- **`parentIsLocal` = true**：变量在**本帧**的局部槽。第一次捕获时把槽位值装进新 `Upvalue` 盒子，
  并把 `locals[parentIndex]` **原地替换**成这个盒子（这就是 §4.2 说的"槽里可能是盒子"的来源）。
  **后续再有闭包捕获同一变量，发现槽已是 `Upvalue`，直接复用**——保证多个闭包看到同一盒子。
- **`parentIsLocal` = false**：变量来自更外层，父帧已经把它做成 `Upvalue`，这里直接引用
...
> 这正是"为什么多个闭包共享外部变量"、以及"为什么 `for` 里 `var i` 被所有闭包共享、
> 而 `let i` 每次迭代独立"的实现基础：`let` 在编译器层每轮分配新槽，于是每次捕获的是不同盒子。

### 8.2 LOAD_UPVAL / STORE_UPVAL
...
- **`closureEnv` 优先**：块级 `let/const`（在 `Environment` 链上）优先于 `globalEnv`；查找命中 `has`
  才读（与 D6 §5 的 `Environment` 链一致）。`GlobalIc`（D7 §5）缓存"名字→拥有者 Environment"加速此路径。
...
`return` 覆盖原返回值。`finally` 里的 `return` 会覆盖原返回值（如 `try{return 1}finally{return 2}`
返回 `2`），靠 `STASH_RESULT` 覆盖 `lastResult` 实现。
...
## 14. 迭代协议（for-in / for-of）

**for-in**（`Vm.kt:493`）：`FOR_IN_INIT` 取对象可枚举键列表压栈成 `IterState`；`FOR_IN_NEXT` 每次取一
个键压栈，取完跳 `a`（循环尾）。**for-of**（`Vm.kt:506`）严格走 `Symbol.iterator` 协议：`buildForOfState`
（`Vm.kt:568`）先查 `@@iterator`，取 `next` 方法；`ForOfState.advance`（`Vm.kt:553`）每次调 `next()`
读 `done`/`value`。无迭代器时降级成合成迭代器（字符串/数组/普通对象也能 for-of）。
...
- **调用 = 递归建帧**：`Frame` 封装单次调用的全部状态，调用栈 = Kotlin 递归栈，清晰且便于实现
  闭包、异常跨函数传播；大数组池化抵消递归的分配开销。
...
- **Frame 不是"全局单例"**：每个调用独立 Frame；误用共享可变状态（如把局部当全局）会导致串调用。
