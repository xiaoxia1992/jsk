...
> **白话导读（给第一次读的人）**：VM 就是个"读一条、做一条"的大循环，用 `pc` 记读到第几行。每调一个 JS 函数，就开一份"档案"(Frame)，装着盘子(操作数栈)、格子表(局部变量)、参数、this、闭包盒子。本篇从主循环讲到函数怎么调用、参数怎么传、闭包盒子怎么读写、异常怎么接、for-in/for-of 怎么遍历，全部带源码细节——想先建立直觉，重点看 §1 心智模型和 §7 的 `add(3,4)` 完整走查。
...
> **小白讲解**：把 VM 想成一个"读一条、做一条"的机器人：它有个指针 `pc` 指"当前读到第几行"，还有个"盘子"(`stack`) 临时放操作数（后进先出，像一摞盘子，只能从顶上拿放）。每读一行指令，就按指令的意思动一下盘子，再把 `pc` 往后挪。循环、判断这类"跳来跳去"的指令，就是通过改 `pc` 实现的。下面那段伪代码就是主循环的骨架。
...
> **小白讲解**：这是全篇最值得跟着走一遍的例子：从"调用 `add(3,4)`"到"返回 7"，把建帧、读参数、加法、出帧每一步的盘子和格子变化都列出来了。看不懂前面的硬核内容时，就先啃这一节，建立完整直觉，再回头看 §2/§4/§5 会轻松很多。
...
> **小白讲解**：**闭包** = "函数 + 它定义时能看到的那些外层变量"。VM 实现上，外层变量不按名字找，而是编译期给每个被捕获的变量编个"upvalue 号"，运行时用一个小盒子 `Upvalue` 装着那个变量的值，函数对象里存着这些盒子的引用。这样函数即使被传到别处调用，也能通过盒子找到当初的变量。和 D6 §5 的 `Environment` 对照看，是闭包捕获的两种表示。
