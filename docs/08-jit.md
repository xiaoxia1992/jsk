...
> **写给小白（本章导读）**：**JIT（Just-In-Time，即时编译）** 是让 JS 跑得更快的技术：当某段函数"跑热了"，引擎把它从"一条条解释的字节码"现场翻译成"JVM 能直接跑的机器码近亲（JVM 字节码）"，之后就飞快。
> - **基础信息**：你不用懂 JVM 也能读——§2.4 用 `sum(n)` 一个例子，从原始 JS → 字节码 → JIT 生成的 JVM 指令，一行行对照讲，还教你怎么读 JVM 指令（首字母是类型、后半是动作）。建议直接从 §2.4.0 开始读。
> - **别的方案对比**：① **解释器**（KJS 默认，慢但简单，见 D5）；② **tracing JIT**（只编译"跑得最热的循环路径"，快但复杂，如 LuaJIT）；③ **生成 `.java` 再 `javac`**（慢、依赖文件，KJS 不用，直接内存里用 ASM 拼）。KJS 选 **模板 JIT**：一条字节码 → 一小段固定的 JVM 指令，机械、安全、好验证。
> - **进阶**：§5 讲"为什么快"（六层消除），§7 讲"怎么猜出变量是数字以便特化"，§13 讲整体取舍。
...
> **小白讲解**：一句话：KJS 的 JIT 把"自己的字节码"翻译成"JVM 的字节码"，不生成 `.java` 文件、直接在内存里拼出来。复杂操作（属性、相等、调用）都交给和解释器同一份的 `JitBridge`，JIT 只亲自优化"数字加减"这一类。
...
> **小白讲解**：速度不是来自"更聪明的算法"，而是"一层层拿掉解释器的累赘"：去掉"读下一条指令+大分支分发"、去掉"堆上的数组当栈"、最关键的是"数字全程用原生 double、不装箱"，于是循环里零内存分配。下面对照表把"每轮循环解释器干嘛、JIT 干嘛"列得清清楚楚。
...
> **小白讲解**：这节回答"为什么这么设计"：选模板 JIT 而非 tracing JIT（简单、可静态分析）；只特化数值、其余委托（最难保证正确的交给已验证代码）；不支持就整函数放弃（宁可少优化也不引入半正确）；内存编译不落盘（零依赖、冷启动重编）。 **别的引擎**：V8 走"解释器 Ignition + 多层 JIT TurboFan"，分层更激进；QuickJS 没有 JIT，靠解释器+精巧字节码取胜。
