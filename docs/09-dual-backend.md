...
> **写给小白（本章导读）**：这一章讲 KJS 的一个"作弊式保障"——它同时实现了两套执行引擎（字节码 VM + 树遍历 Walker），让它们互相核对结果，保证语义正确。
> - **基础信息**：**差分测试（对拍）** = 同一段代码用两套实现各跑一遍，结果应一致；不一致就说明至少一套有 bug。Walker 实现得最直白（"见到 if 就执行 if 分支"），几乎不碰字节码那些易错环节，所以把它当"正确答案参考"。
> - **别的方案对比**：① 单实现 + 单元测试（常见，但实现错了测试也跟着错）；② 对照一个独立参考实现（如用 Node 跑同段代码对拍，更权威但要外部依赖）；③ KJS 用"自带的第二套实现当 oracle"，零外部依赖、随时能跑。关键前提是两套**共享同一套值模型**（D6），否则各算各的对拍没意义。
> - **进阶**：§3 看对拍怎么归一化比较，§5 看为什么把控制流用"异常信号"实现。
...
> **小白讲解**：VM（字节码）快但实现复杂、容易出 bug；Walker（遍历语法树）慢但写法直白、几乎不会错。让慢而对的 Walker 当"裁判"，VM 每次改完都和它比，就能立刻发现 VM 的语义偏差。Walker 不是备胎，是"正确性预言机"。
...
> **小白讲解**：对拍核心三步：同一份源码 → 分别用 VM 和 Walker 执行 → 把两边都"拍平"成可比较的字符串（对象按键排序、数组按序）再 `assertEquals`。任何不一致都暴露 VM 的缺陷。 **进阶**：这招是编译器/VM 工程经典做法，V8 也用多后端互验。
...
> **小白讲解**：总结：Walker 当 oracle、共享值模型、控制流用轻量异常信号、能力边界清晰（不支持的特性直接报错，不静默给错结果）、结果归一化比较。每种选择都为了"对拍真能抓到 bug"。
