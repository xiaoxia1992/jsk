...
**内置函数**就是 JS "自带的函数"，比如 `console.log`、`Array.prototype.map`、`JSON.parse`、`Math.sqrt`。这一章讲它们是怎么"挂进" KJS 引擎的，以及命令行怎么驱动引擎。在 KJS 里，一个内置函数就是一个用 Kotlin 写的 lambda，外面套一层 `JsFunction.native(...)` 包装成 JS 函数；VM 在调用时并不区分"你写的函数"和"引擎自带的函数"，都走同一条路，所以加一个内置 = 写个 lambda 挂到全局对象或某个原型上，完全不用改编译器或 VM。这件事有几种做法：把内置写死进字节码/VM（比如把 `console.log` 当特殊 opcode），快但难扩展；用 JS 自身实现内置（比如用 JS 写 `map`），纯但慢、且启动有依赖。KJS 选了用宿主语言注册，灵活、零侵入，并把核心库和扩展库拆成两层文件保持可读。往下 §3.1 看 `for-of` 怎么靠"迭代协议"统一数组 / Map / Set 的遍历，§3.3 注意 Promise 目前是"同步模拟"的已知简化。
...
核心其实只有一句：一个内置函数，就是一个 `native` 函数（用 Kotlin 的 lambda 包成 JS 函数）。VM 在调用时并不区分"你写的函数"和"引擎自带的函数"，统一走 `JsFunction.call`。所以"挂一个内置"这件事，本质上只是 `target.set("map", nativeFn)` 这么一行，干净利落。
...
命令行入口极其简单：`kjs 文件.js` 读文件执行，`kjs -e '代码'` 执行内联代码，不带参数则进入 REPL 交互。最值得记住的是 `kjs --trace -e '...'`——它会把"词法 → 语法 → 编译 → VM 步进"全过程打印出来，是边读文档边看真实执行流的最佳方式。
...
这一节总结设计取舍：内置用 native 函数，做到零侵入；核心库与扩展库分层存放，保持可读；迭代协议统一；Promise 目前是同步模拟的已知简化；CLI 极简。代价则是 Promise 的时序不符合标准、扩展库需要手动维护。
