...
这套文档是 KJS 引擎的源码剖析，也就是"一个 JavaScript 引擎到底是怎么写出来的"。如果你没有编译原理或虚拟机的基础，推荐按下面的顺序读——每一篇都从最具体的例子讲起，难懂的词会在第一次出现时就地解释：

1. `00-overview.md`：先用最小例子讲清"你写的代码是怎么变成运行结果的"；
2. `architecture.md`：整台引擎的全景地图，认识词法、语法、编译、虚拟机各管什么；
3. `01-lexer.md` → `02-parser-ast.md` → `03-bytecode.md` → `04-compiler.md` → `05-vm.md`：流水线的五道工序，一层层往下走；
4. `06-runtime-values.md` → `07-inline-cache.md` → `08-jit.md` → `09-dual-backend.md` → `10-intrinsics-cli.md`：进阶主题，分别讲值怎么存、属性访问怎么加速、怎么进一步提速、怎么保证正确、内置函数怎么接。

一个 JS 引擎，本质是把 JS 代码"翻译"成计算机能跑的东西，再一步步执行。KJS 走的是"翻译成字节码 + 虚拟机解释执行"这条路，跑热了再用 JIT 翻译成更快的 JVM 字节码。思路与 V8 等主流引擎相似，细节各有取舍——文档各章都会顺带对比别的方案。
