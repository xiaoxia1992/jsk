...
> **写给小白（先看这段）**：这套文档是"KJS 引擎源码剖析"，也就是"一个 JavaScript 引擎是怎么写出来的"。如果你没学过编译器/虚拟机，建议按下面顺序读，每篇开头都有"写给小白"导读，把难词先翻译成人话：
> 1. `00-overview.md` —— 先用比喻讲清"程序怎么从你写的代码变成屏幕上的结果"；
> 2. `architecture.md` —— 整台引擎的"地图"，认识词法/语法/编译/虚拟机各管啥；
> 3. `01-lexer.md` → `02-parser-ast.md` → `03-bytecode.md` → `04-compiler.md` → `05-vm.md` —— 流水线的五道工序，一层层往下走；
> 4. `06-runtime-values.md` → `07-inline-cache.md` → `08-jit.md` → `09-dual-backend.md` → `10-intrinsics-cli.md` —— 进阶主题：值怎么存、怎么加速、怎么更快、怎么保证正确、内置函数怎么接。
> **基础信息**：一个 JS 引擎 = 把 JS 代码"翻译"成计算机能跑的东西，再一步步执行。KJS 的做法是"翻译成字节码 + 用虚拟机解释执行"，跑热了再用 JIT 翻译成更快的 JVM 字节码。别的引擎（如 V8）思路类似、细节不同，文档各章都做了"和别的方案对比"。
