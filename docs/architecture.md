...
如果把 KJS 看成一座工厂，本章就是它的全景地图。工厂里有几类工位：词法分析器（Lexer）把原始文字切成词；语法分析器（Parser）把这些词拼成结构树；编译器（Compiler）把结构树翻译成字节码；虚拟机（VM）照字节码执行。除此之外，"全局环境"里装着 `console`、`Math` 这类内置功能，"值模型"规定了数字、字符串、对象在内存里到底是什么样子。先把这几块角色记在心里，后面每一章都是在放大其中一个工位。
