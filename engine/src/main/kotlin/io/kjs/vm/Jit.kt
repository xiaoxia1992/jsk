...
object Jit {
    ...
    fun compile(bc: Bytecode): Compiled {
        ...
        while (true) {
            ...
            // Static pool fields. Generated bytecode uses GETSTATIC + AALOAD + SIPUSH
            // to pull constants/strings straight out of these arrays, avoiding a
            // method call through JitBridge.constOf/strOf on every LOAD_CONST/STR.
            cw.visitField(ACC_PUBLIC or ACC_STATIC, "CONSTS", "[L$OBJECT;", null, null).visitEnd()
            cw.visitField(ACC_PUBLIC or ACC_STATIC, "STRINGS", "[L$OBJECT;", null, null).visitEnd()
            ...
            try {
                ...
                m.visitCode()
                emitBody(m, bc, doubleLocals, className)
                m.visitMaxs(-1, -1); m.visitEnd()
                cw.visitEnd()
            ...
            // Wire the constant / string pools into the freshly-loaded class.
            cls.getDeclaredField("CONSTS").set(null, bc.constants.toTypedArray())
            cls.getDeclaredField("STRINGS").set(null, bc.strings.toTypedArray() as Array<Any?>)
    ...
    private fun emitBody(m: MethodVisitor, bc: Bytecode, doubleLocals: BooleanArray, className: String) {
        val LV_CLOSURE = 3
        val LV_THIS = 4
        val LV_ARGS = 5
        ...
        for (pc in 0 until bc.size) {
            ...
            when (op) {
                ...
                Op.LOAD_CONST -> {
                    ...
                    when (v) {
                        ...
                        is Boolean -> { m.visitInsn(if (v) ICONST_1 else ICONST_0); apush(T.BOOL) }
                            ...
                            // GETSTATIC cls.CONSTS; SIPUSH idx; AALOAD
                            m.visitFieldInsn(GETSTATIC, className, "CONSTS", "[L$OBJECT;")
                ...
                Op.LOAD_STR -> {
                    m.visitFieldInsn(GETSTATIC, className, "STRINGS", "[L$OBJECT;")
                    m.visitIntInsn(SIPUSH, a)
                    m.visitInsn(AALOAD)
                    apush(T.ANY)
                }
